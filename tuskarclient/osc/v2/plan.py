...
class SetManagementPlan(show.ShowOne):
    ...
    def take_action(self, parsed_args):
        ...
        patch.extend(utils.flavors_args_to_patch(parsed_args.flavors, roles))
        patch.extend(utils.scale_args_to_patch(parsed_args.scales, roles))

        if len(patch) > 0:
            plan = client.plans.patch(parsed_args.plan_uuid, patch)
        else:
            print(("WARNING: No valid arguements passed. No update operation "
...
class AddManagementPlanRole(show.ShowOne):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            'plan_uuid',
            help="The UUID of the plan to add Role."
        )
...
class RemoveManagementPlanRole(show.ShowOne):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            'plan_uuid',
            help="The UUID of the plan to remove the Role."
        )

        parser.add_argument(
            'role_uuid',
            help="The UUID of the Role being removed to the Plan."
        )
