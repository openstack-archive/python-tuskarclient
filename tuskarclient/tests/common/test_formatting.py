...
class PrintTest(tutils.TestCase):
    ...
    def test_print_list(self, mock_out):
        ...
        fmt.print_list(objs, fields, formatters, custom_labels)
        self.assertEqual(
            ('+------+-------+-----------------+\n'
...
class FormattersTest(tutils.TestCase):
    ...
    def test_attr_formatter_chained(self):
        obj = mock.Mock()
        obj.letters = ['a', 'b', 'c']
        letters_formatter = fmt.attr_proxy('letters', fmt.len_formatter)
        self.assertEqual(3, letters_formatter(obj))

    def test_len_formatter(self):
        list_ = ['a', 'b', 'c']
        self.assertEqual(3, fmt.len_formatter(list_))
